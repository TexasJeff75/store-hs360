...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (openaiKey) {
      ...
      description = await generateWithBuiltIn(prompt, systemPrompt);
    }
...
async function generateWithBuiltIn(prompt: string, systemPrompt: string): Promise<string> {
  try {
    // @ts-ignore - Supabase.ai is available in the edge runtime
    const model = new Supabase.ai.Session("mistral");
    const fullPrompt = `${systemPrompt}\n\n${prompt}`;
    const output = await model.run(fullPrompt);
...
function generateFallbackDescription(prompt: string): string {
  ...
  const structureMatch = prompt.match(/following HTML format:\n([\s\S]*?)\n\nImportant guidelines/);
  if (structureMatch) {
    let html = structureMatch[1];
    html = html.replace(/\[2-3 sentences[^\]]*\]/g,
      `${name} is a professional-grade ${category.toLowerCase()} product${brand ? ` by ${brand}` : ""} designed for healthcare practitioners and their patients. This formulation is crafted to meet the highest standards of quality and efficacy.`);
    html = html.replace(/\[1-2 sentences[^\]]*\]/g,
      `Manufactured in a GMP-certified facility with rigorous quality control standards.`);
    html = html.replace(/\[Benefit \d+\]/g, (_, i) => {
      const defaults = [
      ...
      return defaults[i] || "Premium quality product";
    });
    html = html.replace(/\[[^\]]+\]/g,
      `${name} is formulated to support optimal health outcomes for practitioners and their patients.`);
    return html;
  }
