/*
  # Fix Commission Calculation for Products with Markup

  ## Issue
  Products with markup (like ProxiGene) should receive 100% of the margin, not have 
  commission rates applied to the base margin. The markup itself already represents 
  100% commission intent.

  ## Change
  When an item has a markup > 0, the base margin should also be 100% commission 
  (not subject to commission rate). This ensures products marked for special pricing 
  give the full margin to the sales rep.

  ## Example
  ProxiGene: $249 cost, $199 price, $50 markup x 25 qty
  - Base margin: ($199 - $249) * 25 = -$1250 (but we use price+markup)
  - Actual price: $199 + $50 = $249 per unit → $6225 total
  - Cost: $249 per unit → $6225 total  
  - Effective price for commission: ($199 + $50) = $249
  - Margin: ($249 - $199) * 25 = $1250
  - Commission: 100% of $1250 = $1250 (not 40% of $1250)
*/
...
-- Create updated commission calculation function with proper markup handling
    ...
    IF v_commission_rate IS NOT NULL THEN
        ...
        LOOP
          ...
          IF v_base_margin > 0 THEN
            ...
            IF v_item_markup > 0 THEN
              -- When item has markup, it gets 100% of the total margin (base + markup)
              -- This is because markup indicates special/contract pricing
              v_markup_amount := v_item_markup * v_item_quantity;
              v_markup_commission := v_markup_amount;
              -- Base margin also gets 100% when there's a markup
              v_base_commission := v_base_margin;
              v_total_item_commission := v_base_commission + v_markup_commission;
            ELSE
              -- No markup: apply commission rate to base margin
      ...
      IF v_distributor_id IS NOT NULL THEN
        ...
        ELSIF v_commission_split_type = 'fixed_with_override' THEN
          ...
          -- Add markup commission to sales rep (100% of markup always goes to sales rep)
          IF NEW.items IS NOT NULL THEN
            FOR v_item IN SELECT * FROM jsonb_array_elements(NEW.items)
            LOOP
              v_item_markup := COALESCE((v_item->>'markup')::numeric, 0);
